...
```bash
ls -1v *.PART* | sed "s/^/file '/;s/$/'/" > list.txt && ffmpeg -y -f concat -safe 0 -i list.txt -c copy "$(grep PART1 list.txt | sed "s/file '//;s/'$//;s/.PART1././")"
```

### Windows (PowerShell)

```powershell
gci *PART* | sort Name | % { "file '$($_.Name)'" } | sc list.txt && ffmpeg -y -f concat -safe 0 -i list.txt -c copy ((Select-String PART1 list.txt).Line -replace "file '|'|\.PART1\.",'')
```
